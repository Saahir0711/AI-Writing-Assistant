...
def generate_essay_content(details):
    try:
        ...
        temp = 0.3
    intro_p = f"Write an indroduction for an {details["essay_type"]} essay about {details["topic"]} on the topic of {details["length"]}."
    ...
    choice = input("> ").strip()
    if choice == "1":
        body_p = f"Write a full body for an essay on {details["topic"]} with the stance of details {details["target_audience"]}."
    ...
    concl_p = f"Write a conclusion for an {details["essay_type"]} essay about {details["topic"]} with the seance of {details["target_audience"]}."
